...
void fillTRKLayerAtECAL_with_EEprojTOB( TH2F *hEvt_EE_SUBDET, std::vector<float> & vSUBDET_ECAL_, TH2F *hSUBDET_ECAL, int ieta_global_offset, int ieta_signed_offset ){
...
void fillTRKLayerAtECAL_with_EEprojTOB( TH2F *hEvt_EE_SUBDET[][nEE], std::vector<float> vSUBDET_ECAL_[][Nhitproj], TH2F *hSUBDET_ECAL[][Nhitproj], int nSUBDET, unsigned int proj ){
  ...
  for(int nLayer=0; nLayer<nSUBDET; nLayer++){
    ...
    fillTRKLayerAtECAL_with_EEprojTOB(hEvt_EE_SUBDET[nLayer][0], vSUBDET_ECAL_[nLayer][proj], hSUBDET_ECAL[nLayer][proj], ieta_global_offset, ieta_signed_offset);
    ...
    fillTRKLayerAtECAL_with_EEprojTOB(hEvt_EE_SUBDET[nLayer][1], vSUBDET_ECAL_[nLayer][proj], hSUBDET_ECAL[nLayer][proj], ieta_global_offset, ieta_signed_offset);
  }
}

void fillTRKLayerAtEBTOB (DetId id, int layer_, unsigned int proj, TH2F *hSUBDET_ECAL[][Nhitproj], std::vector<float> vSUBDET_ECAL_[][Nhitproj] ) {
...
void fillHelperAtEETOB ( float phi_, float eta_, int layer_, TH2F *hEvt_EE_SUBDET[][nEE]) {
  int iz_ = (eta_ > 0.) ? 1 : 0;
 // hEvt_EE_SUBDET[layer_-1][iz_]->Fill( phi_, eta_);
}

unsigned int DetFrameProducer::getLayerTOB(const DetId& detid, const TrackerTopology* tTopo) {
 //                                            uint16_t
 // +------------+---------------+---------------------------+-----------------+----------------+
  ...
  if (proj==0){
	layer_ = iL + 1;
      for ( int iz(0); iz < nEE; iz++ ) {
      	...
      	sprintf(hname, "evt_TOB_layer%d_EE%s",layer_,zside);
...
//    std::cout << "Strip Id = " << id << " - subId ( 3->TIB | 4->TID | 5->TOB | 6->TEC ) = " << subid << " - Layer = " << layer <<  std::endl;
    ...
    for(stripiter=rechitRangeIteratorBegin;stripiter!=rechitRangeIteratorEnd;++stripiter){//loop on the rechit
     ...
     //   std::cout << " " << iRecHit << " | global position: x = " << GP.x() << " , y = "<< GP.y() << " , z = " << GP.z() <<std::endl;
        ...
        if ( ecalId.subdetId() == EcalBarrel ){
            fillTRKLayerAtEBTOB ( ecalId, layer, proj, hTOB_ECAL, vTOB_ECAL_ );
	    }
        else if ( ecalId.subdetId() == EcalEndcap ){
            fillHelperAtEETOB ( phi, eta, layer, hEvt_EE_TOB);
        }
  ...
  fillTRKLayerAtECAL_with_EEprojTOB( hEvt_EE_TOB, vTOB_ECAL_, hTOB_ECAL, nTOB, proj);
