...
//  jetCollection_sel = iConfig.getParameter<std::string>("jetCollection");//
 /* if (jetCollection_sel == "ak4"){
  ...
  doEvtSeljetDijetTau = iConfig.getParameter<bool>("doEvtSeljetDijetTau");	
...
DetFrameProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   ...
   e2e::Frame2D vEvtSeljetDijetTau_reshaped (nDetFrameH, e2e::Frame1D (nDetFrameW,0.));
   ...
   if (doEvtSeljetDijetTau){
        vChannelMap[11] = 1;
        fillEvtSeljetDijetTau (iEvent, iSetup );
